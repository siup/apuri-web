...
- Subdomena `stormbot.apuri.pl` — rekord A na to samo IP VPS (lub CNAME do `apuri.pl`)
- Planner działa pod ścieżką `https://apuri.pl/planner/` (bez osobnej subdomeny)
...
```env
VIRTUAL_HOST=apuri.pl,www.apuri.pl,stormbot.apuri.pl
LETSENCRYPT_HOST=apuri.pl,www.apuri.pl,stormbot.apuri.pl
...
Ten sam użytkownik/hasło chroni `/planner/` oraz link `/stormbot/` na stronie głównej (przekierowanie na subdomenę). Samą subdomenę `stormbot.apuri.pl` można otworzyć bez hasła, jeśli zna się adres.
...
sudo certbot --nginx -d apuri.pl -d www.apuri.pl -d stormbot.apuri.pl
```
...
- Na VPS z hostowym Nginx: `sudo certbot --nginx -d apuri.pl -d www.apuri.pl -d stormbot.apuri.pl`
...
- Hostowy Nginx musi przekazywać subdomeny do kontenera — w `server_name` powinny być: `apuri.pl www.apuri.pl stormbot.apuri.pl` (plik `deploy/nginx/apuri.pl.conf`)
...
Gdy Planner lub Stormbot będą gotowe jako osobne kontenery, w `docker/nginx.conf` zamień serwowanie plików statycznych w `/planner/` lub bloku `stormbot.apuri.pl` na `proxy_pass` do odpowiedniego backendu.
